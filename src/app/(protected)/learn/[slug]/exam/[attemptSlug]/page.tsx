...
import {
    useGetAttemptAcademyExam,
    usePostAnswerAcademyExam,
    usePostAnswerAcademyExamFile,
    usePostSubmitAcademyExam,
} from "@/lib/queries/academy"
import { CodeMetadata } from "@/types/attempt"
...
const AcademyExam: React.FC<QuizContainerWrapperProps> = ({
    ...
    } = useGetAttemptAcademyExam(slug, attemptSlug)

    const postAnswerMutation = usePostAnswerAcademyExam(
        slug,
        attempt?.id_attempt || "",
    )
    const postAnswerFileMutation = usePostAnswerAcademyExamFile(
        slug,
        attempt?.id_attempt || "",
    )
    const submitMutation = usePostSubmitAcademyExam(
        slug,
        attempt?.id_attempt || "",
    )
    ...
    return (
        <div className="container py-11 flex flex-col gap-8">
            <QuizContainer
                ...
                basePath={basePath}
                showRemainingTime={false} // Disable timer for Academy exams
            />
        </div>
    )
}

export default AcademyExam
